...
path = """~/3epwzqki/3epwzqki/accelerometer/*.csv"""
...
# %load_ext sql
# %sql postgresql://postgres:secret@localhost/beiwe
```
...
class ModAccel:
    ...
    and make a new derived table
    utc_time | utc_time_day | utc_time_hour | mag | patientid
    """
...
am = ModAccel(db = 'beiwe', user = 'postgres', password = 'secret')
am.deriveTable()
```
...
#\copy (SELECT * FROM labeled_features2) to '~/labeled_features2.csv' csv header
```
